...
@implementation ParserOperation
@synthesize urlTopGame;
@synthesize completionBlock, errorHandler;
...
- (void)main {
    ...
    NSError *error = nil;
    if (self.urlTopGame) {
        dataXML = [NSURLConnection sendSynchronousRequest:[NSURLRequest requestWithURL:self.urlTopGame] returningResponse:nil error:&error];
    ...
    if (!dataXML) {
        if (self.errorHandler) {
            self.errorHandler([NSError errorWithDomain:@"ParseerOperation" code:0 userInfo:[NSDictionary dictionaryWithObject:@"No Data available to Parser" forKey:NSLocalizedDescriptionKey]]);
        }
